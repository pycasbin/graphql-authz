...
class TestConfig(TestCase):
    ...
    def test_graphql_middleware(self):
        ...
        response = graphql_sync(
            schema, query, middleware=[casbin_middleware], context_value={"role": "user"})

        self.assertEqual(response.errors[0].formatted, {
            "message": "user can not query project.name",
            "path": ['project', 'name'],
            "locations": [{'line': 3, 'column': 20}]
        })

        self.assertEqual(response.errors[1].formatted, {
        ...
        self.assertEqual(response.errors[2].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 0, 'tickets', 1, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[3].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 0, 'tickets', 2, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[4].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 0, 'tickets', 3, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[5].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 1, 'tickets', 0, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[6].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 1, 'tickets', 1, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[7].formatted, {
            "message": "user can not query project.members.tickets.message",
            "path": ['project', 'members', 1, 'tickets', 2, 'message'],
            "locations": [{'line': 5, 'column': 28}]
        })

        self.assertEqual(response.errors[8].formatted, {
            "message": "user can not query project.members.tickets.message",
    ...
    def test_graphql_middleware_unauthorized_querying_non_nullable_fields(self):
        ...
        response = graphql_sync(schema, query, middleware=[casbin_middleware],
